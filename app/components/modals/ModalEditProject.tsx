...
const ModalEditProject = ({
  ...
  const OnSubmit: SubmitHandler<Proj> = async (data) => {
    ...
    try {
      ...
      if (data.projectPeriod === "ONGOING") {
        myProject.deadlineDate = "";
      } else {
        myProject.deadlineDate = data.deadlineDate
          ? data.deadlineDate.toISOString().slice(0, 10)
          : "";
      }
      // let techNames: TechnologyStackDto[] = technologyNames.map((name) => ({
      //   technologyName: name,
      // }));
      // techNames.unshift({ technologyName: data.technologyName });
      ...
      //console.log("TECH NAMES facut: ", techNames);
  ...
  useEffect(() => {
    if (project) {
      ...
      setValue("noOfMembers", project.teamRoles[0].noOfMembers);
      // console.log("projectName:", project.projectName);
      // console.log("tech stack:", project.technologyStack);
      // console.log("team roles:", project.teamRoles);

      setCount.set(project.technologyStack.length - 1);
      setCount1.set(project.teamRoles.length - 1);
      ...
      // console.log("techNAmes:", technologyNames);
      // console.log("teamRoles:", teamRolesRowsData);
    }
  }, [project]);
  ...
  const teamRolesRows = Array(count1)
    ...
    .map((_, index) => (
      <Group key={index}>
        <Select
          value={
            index >= 0 && index < teamRolesRowsData.length
              ? teamRolesRowsData[index].role
              : 0
          }
        ...
        <NumberInput
          value={
            index >= 0 && index < teamRolesRowsData.length
              ? teamRolesRowsData[index].noOfMembers
              : 0
          }
